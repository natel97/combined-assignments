...
public class SimplifiedRational implements IRational {
	...
	public SimplifiedRational(int numerator, int denominator) throws IllegalArgumentException {
		...
		if (numerator != 0) {
			...
			_numerator = simplify(numerator, denominator)[0];
			_denominator = simplify(numerator, denominator)[1];
		} else {
	...
	public boolean equals(Object obj) {
		...
		if (_numerator != rat.getNumerator() || _denominator != rat.getDenominator()) {
			return false;
		}
		return true;
	}
	...
	public String toString() {
		...
		return (_numerator < 0 != _denominator < 0) ? "-" + Math.abs(_numerator) + "/" + Math.abs(_denominator)
				: Math.abs(_numerator) + "/" + Math.abs(_denominator);
